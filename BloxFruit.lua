...
v24.Parent = v23;
v24.CornerRadius = UDim.new(0, 16);
v25.Parent = v23;
v25.LightEmission = 1;
...
v16.Home:AddSection("Bản Quyền Thuộc Về Duca Roblox");
v16.Home:AddButton({
    Title = "Youtube",
...
spawn(function()
    while task.wait(0.1) do
        if (AutoFarmMasDevilFruit and (TypeMastery == "Quái Gần")) then
            pcall(function()
                for v842, v843 in pairs(game.Workspace.Enemies:GetChildren()) do
                    if (v843.Name and v843:FindFirstChild("Humanoid") and v843:FindFirstChild("HumanoidRootPart")) then
                        if ((game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v843:FindFirstChild("HumanoidRootPart").Position).Magnitude <= 5000) then
                            ...
                            until not AutoFarmMasDevilFruit or (not MasteryType == "Quái Gần") or not v843.Parent or (v843.Humanoid.Health == 0)
                            bringmob = false;
                            _G.UseSkill = false;
                        end
                    end
    ...
    v505.RenderStepped:Connect(function()
        ...
        if v594 then
            ...
            if not v513 then
                v14:Notify({
                    Title = "Thông Báo",
                    Content = "Đảo Núi Lửa Đã Tìm Thấy",
                    Duration = 10
                });
                v513 = true;
            end
    ...
    v505.RenderStepped:Connect(function()
        ...
        if v616 then
            ...
            if not v513 then
                v14:Notify({
                    Title = "Thông Báo",
                    Content = "Đảo Leviathan Đã Tìm Thấy",
                    Duration = 10
                });
                v513 = true;
            end
                ...
                if (_G.BringMob and bringmob) then
                    if ((v734.Name == MonFarm) and v734:FindFirstChild("Humanoid") and (v734.Humanoid.Health > 0)) then
                        if (v734.Name == "Factory Staff") then
                            if ((v734.HumanoidRootPart.Position - FarmPos.Position).Magnitude <= 1000000000000000) then
...
local v93 = v16.Setting:AddSection("Chọn Kĩ Năng Để Dùng");
    ...
    while task.wait() do
        pcall(function()
            if _G.WalkonWater then
                game:GetService("Workspace").Map["WaterBase-Plane"].Size = Vector3.new(3000, 115, 3000);
            else
                game:GetService("Workspace").Map["WaterBase-Plane"].Size = Vector3.new(3000, 115, 3000);
            end
        end);
...
local v140 = v16.Raid:AddToggle("ToggleNextIsland", {
    ...
    Description = "Lỗi Thì Tắt Bật Lại",
    Default = false
});
...
local v56 = v16.Race:AddSection("Tộc V4");
v16.Race:AddButton({
    Title = "Đến Cửa Tộc",
    Description = "",
    Callback = function()
    ...
    spawn(function()
        pcall(function()
            while wait() do
                ...
                if (v811 == 5) then
                    v561:SetDesc("Leviathan đang ở ngoài kia");
                elseif (v811 == 0) then
                    v561:SetDesc("Tôi không biết");
                else
...
local v156 = v16.Sea:AddSection("Dragon Dojo");
...
local v159 = v16.Sea:AddParagraph({
    Title = "Trạng Thái Nhiệm Vụ Cầu Đỏ",
    Content = "✅️"
});
spawn(function()
    pcall(function()
