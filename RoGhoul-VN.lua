local gui = loadstring(game:HttpGetAsync("https://raw.githubusercontent.com/z4gs/scripts/master/testtttt.lua"))():AddWindow("Ro-Ghoul By Duca Roblox", {
...
local tab1, tab2, tab3, tab4 = gui:AddTab("Sảnh"), gui:AddTab("Chọn Kiểu Cày"), gui:AddTab("Luyện Tập"), gui:AddTab("Khác")
...
local drop = tab1:AddDropdown("Lựa chọn", function(opt)
...
btn = tab1:AddButton("Bắt Đầu", function()
    if not array.autofarm then
        if key then
            btn.Text, array.autofarm = "Dừng Lại", true
            local farmtick = tick()
            while array.autofarm do
                labels("tfarm", "Thời gian đã trôi qua: "..os.date("!%H:%M:%S", tick() - farmtick))
                wait(1)
            end
        else
            player:Kick("Không lấy được Key , vui lòng thử chạy lại tập lệnh")
        end
    else
...
labels = setmetatable({
    ...
    Quest = {prefix = "Nhiệm vụ hiện tại: ", label = tab1:AddLabel("Nhiệm vụ hiện tại: Không có")},
    ...
    Kills = {prefix = "Giết: ", label = tab1:AddLabel("Giết: 0"), value = 0} 
}, {
...
btn3 = tab1:AddButton("Khởi Động Lại", function() labels() end)

if team == "CCG" then tab2:AddLabel("Quinque Stage") else tab2:AddLabel("Giai đoạn Kagune") end
...
tab2:AddSwitch("Cày danh tiếng", function(bool) 
    myData.ReputationFarm = bool
end):Set(myData.ReputationFarm)

tab2:AddSwitch("Rút tiền danh tiếng tự động", function(bool)
    myData.ReputationCashout = bool
end):Set(myData.ReputationCashout)

for i,v in pairs(array.boss) do
    tab2:AddSwitch(i.." Giết Trùm ".."(".."lvl "..v.."+)", function(bool)
        myData.Boss[i] = bool
    end):Set(myData.Boss[i])
end

tab2:AddSlider("Tốc Độ Dịch Chuyển", function(x)
    myData.TeleportSpeed = x
end, {min = 90, max = 250}):Set(45)

tab2:AddSlider("Khoảng cách từ NPC", function(x)
    myData.DistanceFromNpc = x * -1
end, {min = 0, max = 8}):Set(65)

tab2:AddSlider("Khoảng cách từ Trùm", function(x)
    myData.DistanceFromBoss = x * -1
end, {min = 0, max = 15}):Set(55)

labels.p = {label = tab3:AddLabel("Huấn luyện viên hiện tại: "..player.PlayerFolder.Trainers[team.."Trainer"].Value)}

local progress = tab3:AddSlider("Quá Trình", nil, {min = 0, max = 100, readonly = true})
...
player.PlayerFolder.Trainers[team.."Trainer"].Changed:connect(function()
    labels("p", "Huấn luyện viên hiện tại: "..player.PlayerFolder.Trainers[team.."Trainer"].Value)
    progress:Set(player.PlayerFolder.Trainers[player.PlayerFolder.Trainers[team.."Trainer"].Value].Progress.Value)
end)

btn2 = tab3:AddButton("Bắt Đầu", function()
    if not array.trainer then
        array.trainer, btn2.Text = true, "Dừng Lại"
        ...
        while array.trainer do
            ...
            if result == "ĐANG ĐÀO TẠO" then
                for i,v in pairs(workspace.TrainingSessions:GetChildren()) do
                    if waitforobj(v, "Player").Value == player then
                        fire(waitforobj(v, "Comm"), "Hoàn thành", tkey, false)
                        break
                    end
                end
            elseif result == "ĐÀO TẠO HOÀN THÀNH" then
                labels("time", "Đang chuyển sang huấn luyện viên khác...")
                for i,v in pairs(player.PlayerFolder.Trainers:GetDescendants()) do
                    if table.find(trainers, v.Name) and findobj(v, "Quá Trình") and tonumber(v.Progress.Value) < 100 and tonumber(player.PlayerFolder.Trainers[player.PlayerFolder.Trainers[team.."Trainer"].Value].Progress.Value) == 100 then
                        invoke(remotes.Trainers.ChangeTrainer, v.Name)
                        wait(1.5)
                    end
                end
            else
                labels("time", "Thời gian cho đến buổi đào tạo tiếp theo: "..result)
            end
            wait(1)
        end
        labels("time", "")
...
tab4:AddSwitch("Tự động thêm chỉ số Kagune", function(bool) array.weapon = bool end)
tab4:AddSwitch("Tự động thêm chỉ số Độ bền", function(bool) array.dura = bool end)
tab4:AddSwitch("Tự Động Rời", function(bool) array.kick = bool end)
tab4:AddLabel("Tự động xóa danh sách trắng (nhập 1 tên trên mỗi dòng)")
...
for i,v in pairs(array.skills) do
    tab4:AddSwitch("Tự động sử dụng kỹ năng "..i.." (Lên Trùm)", function(bool)
...
local function getQuest(typ)
    labels("text", "Di chuyển đến NPC nhiệm vụ")
    ...
    if array.autofarm and not array.died and (npc.HumanoidRootPart.Position - player.Character.HumanoidRootPart.Position).Magnitude <= 20 then
        if typ then 
            labels("text", "Đang nhận nhiệm vụ...")
        ...
        pcall(function()
            if player.Character.Humanoid.Health > 0 and player.Character.HumanoidRootPart and player.Character.Remotes.KeyEvent then
                ...
                if npc then
                    ...
                    labels("text", "Di chuyển đến: "..npc.Name)
                    ...
                    if not array.found then
                        ...
                        if array.autofarm and player.Character.Humanoid.Health > 0 then
                            ...
                            if npc.Name ~= "Eto Yoshimura" and not findobj(npc.Parent, "Gyakusatsu") and npc.Name ~= "Gyakusatsu" then  
                                labels("text", "Đang thu thập xác chết...")
                                collect(npc)
                            end
                        end
                    end
                else
                    labels("text", "Không tìm thấy mục tiêu, đang chờ...")
                end
            else
                labels("text", "Đang chờ nhân vật hồi sinh")
                array.died = true
            end
        end)
    else
